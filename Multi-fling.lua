...
StatusLabel.Text = "Pilih target untuk di fling"
...
StopButton.Text = "HENTIKAN FLING"
...
DeselectAllButton.Text = "BATALKAN PILIHAN SEMUA"
...
local function UpdateStatus()
    ...
    if FlingActive then
        ...
        StatusLabel.Text = count .. " target(s) Terpilih" 
        StatusLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
    end
end
...
local function StartFling()
    ...
    local count = CountSelectedTargets()
    if count == 0 then
        StatusLabel.Text = "TIDAK ADA TARGET DIPILIH!"
        wait(1)
        StatusLabel.Text = "Pilih target untuk di fling"
        return
    end
    ...
    FlingActive = true
    UpdateStatus()
    Message("Memulai", "Flinging " .. count .. " targets", 2)
...
UpdateStatus()
-- Success message
Message("Loaded", "R4NG's Multi-Target Fling GUI loaded!", 3)
